...
public class SLAChecker extends Thread implements IPerformanceEvaluator {
	...
	public void run() {
		...
		ex = new ScheduledThreadPoolExecutor(slas.getObligations().getSlo()
				.size());
		...
		for (int i = 0; i < slaslo.size(); i++) {
			...
			if (slaslo.get(i).getType() == slal.Type.RT_QUANTILE_TYPE) {
				...
				for (int k = 0; k < size; k++) {
					float q = (slaslo.get(i).getValue().getPair().get(k)
							.getQuantile() / 100.0f);
					quantile[k] = q;

					int responseTime = slaslo.get(i).getValue().getPair()
							.get(k).getResponseTime();
						...
						new Runnable() {
							public void run() {
								...
								for (int j = 0; j < responseTimes.length; j++) {
									int responseTime2 = slo.getValue()
											.getPair().get(j).getResponseTime();
	...
	public void execute() {
		...
		for (int i = 0; i < this.slas.getObligations().getSlo().size(); i++) {
			long[] quantiles = new long[this.slas.getObligations().getSlo()
					.get(i).getValue().getPair().size()];
			for (int k = 0; k < this.slas.getObligations().getSlo().get(i)
					.getValue().getPair().size(); k++) {
				quantiles[k] = this.slas.getObligations().getSlo().get(i)
						.getValue().getPair().get(k).getResponseTime();
			}
			guis[i] = new SLACheckerGUI(
					"ServiceID: "
							+ this.slas.getObligations().getSlo().get(i)
									.getServiceID(), 30000, quantiles[0],
