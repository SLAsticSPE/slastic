...
public class QuantileCalculator {
	 ...
	 * 
	 * @param quantiles
	 *            array of that identifies the quantiles in percent
	 * @param serviceID
	 *            ID that identified the services for which the quantiles should be calculated
	 * @return array of results
	 * @throws ServiceIDDoesNotExistException
	 */
	public long[] getResponseTimeQuantiles(final float[] quantiles, final int serviceID) throws ServiceIDDoesNotExistException {
		long[] retVal;
		...
		final ConcurrentSkipListSet<SLOMonitoringRecord> rtSet = this.mng.getResponseTimes(serviceID); // never null
		final SLOMonitoringRecord[] rtArr = rtSet.toArray(new SLOMonitoringRecord[0]);
		...
		for (int i = 0; i < retVal.length; i++) {
			try {
				...
				if (((rtArr.length % (1.0f / (quantiles[i]))) != 0.0f)) {
					final int index = (int) ((rtArr.length + 1) * (quantiles[i])) - 1;
					...
					final int index = (int) ((rtArr.length) * (quantiles[i])) - 1;
