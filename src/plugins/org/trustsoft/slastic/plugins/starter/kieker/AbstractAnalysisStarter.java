...
public abstract class AbstractAnalysisStarter {
	...
	public boolean start() {
		...
		AbstractAnalysisStarter.log.debug("startReader terminated with"
				+ logReplayRes);
	...
	private boolean startReader() {
		...
		String controllerConfigurationFN = "UNASSIGNED";
		try {
			controllerConfigurationFN = File.createTempFile("monitoring", ".properties").getAbsolutePath();
		...
		return this.startReader(controllerConfigurationFN);
	}

	protected abstract boolean startReader(
			String controllerConfigurationFN);
