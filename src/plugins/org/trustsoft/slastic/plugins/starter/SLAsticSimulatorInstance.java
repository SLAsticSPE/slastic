...
public class SLAsticSimulatorInstance {
    ...
    private void initFromProps() throws IllegalArgumentException {
        try {
            this.fsReaderInputDir = this.props.getProperty(SLAsticSimulatorInstance.PROP_NAME_FSREADER_INPUTDIR);
    ...
    public void run() {
        ...
        this.tpanInstance.addRecordConsumer(new IKiekerRecordConsumer() {
            ...
            final IKiekerRecordConsumer delegate = simCtrl;
            ...
            public void terminate() {
                try {
                    delegate.consumeMonitoringRecord(SimulationController.TERMINATION_RECORD);
                } catch (RecordConsumerExecutionException ex) {
                    log.error("Failed to send no more external records event", ex);
                }
