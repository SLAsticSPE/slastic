...
public class SLAsticSimulatorInstance {
	...
	private static final Log log = LogFactory
			.getLog(SLAsticSimulatorInstance.class);
	private Properties props;
	private static final String PROP_NAME_PREFIX = "slastic.simulation";
	private static final String PROP_NAME_FSREADER_INPUTDIR = SLAsticSimulatorInstance.PROP_NAME_PREFIX
			+ ".fsreader.inputdir";
	...
	private TpanInstance tpanInstance;
	...
	private void initFromProps() throws IllegalArgumentException {
		try {
			this.fsReaderInputDir = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_FSREADER_INPUTDIR);
			this.fsReaderRTMode = this.props.getProperty(
					SLAsticSimulatorInstance.PROP_NAME_FSREADER_RTMODE, "")
					.trim().toLowerCase().equals("true");
			this.fsReaderRTNumThreads = Integer
					.parseInt(this.props
							.getProperty(SLAsticSimulatorInstance.PROP_NAME_FSREADER_RTNUMTHREADS));

			this.reconfPipeName = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_RECONF_PIPENAME);

			if (this.fsReaderInputDir == null
					|| this.fsReaderInputDir.length() == 0) {
				throw new IllegalArgumentException("Missing or empty property "
						+ SLAsticSimulatorInstance.PROP_NAME_FSREADER_INPUTDIR);
			}
			...
			this.pcmRespositoryModel_fn = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_PCM_REPOSITORY_FN);
			this.pcmSystemModel_fn = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_PCM_SYSTEM_FN);
			this.pcmAllocationModel_fn = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_PCM_ALLOCATION_FN);
			this.pcmResourceEnvironmentModel_fn = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_PCM_RESOURCEENV_FN);
			this.slasticReconfigurationModel_fn = this.props
					.getProperty(SLAsticSimulatorInstance.PROP_NAME_SLASTIC_RECONFIGURATIONMODEL_FN);

			this.pcmRepository = PCMModelReader.getInstance().readRepository(
					this.pcmRespositoryModel_fn);
			if (this.pcmRepository == null) {
				SLAsticSimulatorInstance.log
						.error("Failed to read PCM repository from file '"
								+ this.pcmRespositoryModel_fn + "'");
				throw new IllegalArgumentException(
			...
			if (this.pcmSystem == null) {
				SLAsticSimulatorInstance.log
						.error("Failed to read PCM system from file '"
								+ this.pcmSystemModel_fn + "'");
				throw new IllegalArgumentException(
			...
			if (this.pcmAllocation == null) {
				SLAsticSimulatorInstance.log
						.error("Failed to read PCM allocation from file '"
								+ this.pcmAllocationModel_fn + "'");
				throw new IllegalArgumentException(
						"Failed to read PCM allocation from file '"
								+ this.pcmAllocationModel_fn + "'");
			}
			this.pcmResourceEnvironment = PCMModelReader.getInstance()
					.readResourceEnvironment(
							this.pcmResourceEnvironmentModel_fn);
			if (this.pcmResourceEnvironment == null) {
				SLAsticSimulatorInstance.log
						.error("Failed to read PCM resource environment from file '"
								+ this.pcmResourceEnvironmentModel_fn + "'");
				throw new IllegalArgumentException(
						"Failed to read PCM resource environment from file '"
								+ this.pcmResourceEnvironmentModel_fn + "'");
			}
			this.slasticReconfigurationModel = SLAsticModelReader.getInstance()
					.readReconfigurationModel(
							this.slasticReconfigurationModel_fn);
			if (this.slasticReconfigurationModel == null) {
				SLAsticSimulatorInstance.log
						.error("Failed to read SLAstic reconfiguration model from file '"
								+ this.slasticReconfigurationModel_fn + "'");
				throw new IllegalArgumentException(
	...
	public void run() {
		...
		SLAsticSimulatorInstance.log
				.info("Instatiating Simulator with: Repository "
		...
		this.reconfPlanReceiver = new SLAsticSimPlanReceiver(
				this.reconfPipeName, new IReconfPlanReceiver() {
					...
					public void reconfigure(
						...
						SLAsticSimulatorInstance.log
								.info("Sending confirmation to listener "
										+ listener);
		...
		final FSReader r = new FSReader(this.fsReaderInputDir);
		this.tpanInstance = new TpanInstance();
		this.tpanInstance.setLogReader(r);
		this.tpanInstance.addRecordConsumer(new IKiekerRecordConsumer() {

			final IKiekerRecordConsumer delegate = SLAsticSimulatorInstance.this.simCtrl;

			public String[] getRecordTypeSubscriptionList() {
				return this.delegate.getRecordTypeSubscriptionList();
			}
			...
			public void consumeMonitoringRecord(
				...
				this.delegate.consumeMonitoringRecord(monitoringRecord);
			}
			...
			public void terminate() {
				try {
					this.delegate
							.consumeMonitoringRecord(SimulationController.TERMINATION_RECORD);
				} catch (final RecordConsumerExecutionException ex) {
					SLAsticSimulatorInstance.log
							.error(
									"Failed to send no more external records event",
									ex);
				}
		...
		try {
			SLAsticSimulatorInstance.log
					.info("Starting simulator instance ...");
