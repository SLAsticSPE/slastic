...
package org.trustsoft.slastic.tests.junit.framework.properties;
...
public class TestInitProperties extends TestCase {
	...
	private static final String PROP_KEY_SINGLE = TestInitProperties.PROP_PREFIX + "singleValue";
	private static final String PROP_VAL_SINGLE = "iuu4thkjfm,";

	private static final String PROP_KEY_MANY = TestInitProperties.PROP_PREFIX + "multiValue";
	...
	public void testSingleValueProp() {
		...
		{ /* Init properties */
			prop = new Properties();
			prop.put(TestInitProperties.PROP_KEY_SINGLE, TestInitProperties.PROP_VAL_SINGLE);
		}
		...
		{ /* Check if the value is correctly returned by the getProperty method */
			Assert.assertEquals("Unexpected value (getProperty method)", TestInitProperties.PROP_VAL_SINGLE,
					comp.getProperty(TestInitProperties.PROP_KEY_SINGLE));
		}

		{ /* Check if the value is correctly returned by the getProperties method */
			final String[] retVal = comp.getProperties(TestInitProperties.PROP_KEY_SINGLE);
	...
	public void testMultiValueProp() {
		...
		{ /* Init properties */
			prop = new Properties();
			PropertiesFileUtils.putMultiValProp(prop, TestInitProperties.PROP_KEY_MANY,
					TestInitProperties.PROP_VALS_MANY);
		}
		 ...
		 * method
		 */
			Assert.assertEquals("Unexpected value (getProperty method)", TestInitProperties.PROP_VALS_MANY[0],
					comp.getProperty(TestInitProperties.PROP_KEY_MANY));
		}

		{ /* Check if the value is correctly returned by the getProperties method */
			final String[] retVal = comp.getProperties(TestInitProperties.PROP_KEY_MANY);
			Assert.assertEquals("Unexpected array length", TestInitProperties.PROP_VALS_MANY.length, retVal.length);
			Assert.assertTrue(
					String.format("Property values differ: (expected %s; found %s)", TestInitProperties.PROP_VALS_MANY,
							comp.getProperties(TestInitProperties.PROP_KEY_MANY)),
					Arrays.equals(TestInitProperties.PROP_VALS_MANY,
							comp.getProperties(TestInitProperties.PROP_KEY_MANY)));
		}
	}
	...
	public void testNonExistingProp() {
		...
		{ /* Check if getProperty returns null (expected for non-existing properties) */
			Assert.assertEquals("Expected null value for non-existing property (getProperty method)",
					comp.getProperty(TestInitProperties.PROP_KEY_SINGLE), null);
		}
		
		{ /* Check if getProperty returns the default value (expected for non-existing properties) */
			final String defaultValue = "32gds";
			Assert.assertEquals("Expected default value for non-existing property (getProperty method)",
					comp.getProperty(TestInitProperties.PROP_KEY_SINGLE, defaultValue), defaultValue);
		}

		{ /* Check if getProperties returns null (expected for non-existing properties) */
			Assert.assertEquals("Expected null value for non-existing property (getProperties method)",
					comp.getProperties(TestInitProperties.PROP_KEY_MANY), null);
		}
		...
		{ /* Check if getProperties returns the default values (expected for non-existing properties) */
			...
			Assert.assertEquals("Expected default value for non-existing property (getProperties method)",
					comp.getProperties(TestInitProperties.PROP_KEY_MANY, defaultValue), defaultValue);
		}
	}
}

/**
 * Does nothing but returning its property values by means of the methods
 * {@link PropertyComponent#getProperty(String)} and
