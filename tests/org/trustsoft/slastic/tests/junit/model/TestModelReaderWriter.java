...
public class TestModelReaderWriter extends TestCase {
	...
	/**
	 * Registers the two {@link ComponentType}s {@link #componentType0} and
	 * {@link #componentType1}, each having a single {@link Operation}.
	 */
	private void fillSystemModel() {
		...
		tMgr.createAndRegisterOperation(this.componentType0,
				"op0", Boolean.class.getName(), new String[] { void.class.getName() });
		this.componentType1 = tMgr.createAndRegisterComponentType(this.fqnComponentType1);
		tMgr.createAndRegisterOperation(this.componentType1,
				"op1", Integer.class.getName(), new String[] { String.class.getName() });
	}
	...
	private void saveModels() throws IOException {
		...
		this.systemModelManager.saveModels(
				this.tmpSystemModelFile.getAbsolutePath(),
				this.tmpUsageModelFile.getAbsolutePath());
	}
	...
	private void loadModels() throws IOException {
		...
		this.systemModelManagerLoadedModel =
				new ModelManager(this.tmpSystemModelFile.getAbsolutePath(), this.tmpUsageModelFile.getAbsolutePath());
	}
	...
	private void checkResults() {
		...
		for (final OperationCallFrequency opCallFreq : usageModelLoaded.getOperationCallFrequencies()) {
			...
			Assert.assertTrue("Objects contained in system model are not the same!",
					(op == op0Loaded) || (op == op1Loaded));
		}
	}
}
